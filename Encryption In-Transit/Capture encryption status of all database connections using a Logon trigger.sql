...
-- Grant permission for any login to query sys.dm_exec_sessions and sys.dm_exec_connections
IF CAST(CAST(SERVERPROPERTY('productversion') AS CHAR(2)) AS TINYINT) >= 16		-- SQL Server 2022 or above
	EXEC('GRANT VIEW SERVER PERFORMANCE STATE TO [public];');
ELSE
	EXEC('GRANT VIEW SERVER STATE TO [public];');
GO
...
ALTER AUTHORIZATION ON DATABASE::[zDBAEncryptionAuditing] TO [sa]
GO

-- Grant all logins the ability to insert into the table dbo.EncryptionAuditing, by enabling the guest user
GRANT CONNECT, INSERT, UPDATE, SELECT TO [guest];
GO
...
SELECT @EncryptOption = ISNULL(encrypt_option, ''), @ProtocolType = protocol_type
