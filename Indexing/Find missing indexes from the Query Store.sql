-- Lists missing indices from the Query Store during the last week, where there are more than 100 million estimated logical reads in total
...
SELECT mi.DatabaseName,
	   ...
	   STUFF(( SELECT DISTINCT ', ' + c.value('(@Name)[1]', 'sysname')
                ...
                FOR XML PATH('')),1,2,'' ) AS EqualityColumns,
	   STUFF(( SELECT DISTINCT ', ' + c.value('(@Name)[1]', 'sysname')
                FROM StmtXML.nodes('//ColumnGroup') AS t(cg)
                    CROSS APPLY cg.nodes('Column') AS r(c)
                WHERE cg.value('(@Usage)[1]', 'sysname') = 'INEQUALITY'
                FOR XML PATH('')),1,2,'' ) AS InequalityColumns,
	   STUFF(( SELECT DISTINCT ', ' + c.value('(@Name)[1]', 'sysname')
                FROM StmtXML.nodes('//ColumnGroup') AS t(cg)
                    CROSS APPLY cg.nodes('Column') AS r(c)
...
CREATE CLUSTERED INDEX CIX_Output_Missing ON #OutputMissing (QueryPlanHash);

DECLARE IndexCur CURSOR LOCAL FOR
	SELECT DatabaseName, QueryPlanHash, QueryPlan, SampleQueryText
		FROM #OutputMissing
		FOR UPDATE;

OPEN IndexCur;
FETCH IndexCur INTO @DatabaseName, @QueryPlanHash, @QueryPlan, @QuerySQLText;
...
BEGIN
	...
	SET @sql = N'
		SELECT TOP (1) @QueryPlan = CAST(qsp.query_plan AS NVARCHAR(MAX)), @QuerySQLText = qsqt.query_sql_text 
			FROM ' + @DatabaseName + '.sys.query_store_plan AS qsp 
			INNER JOIN ' + @DatabaseName + '.sys.query_store_query AS qsq ON qsp.query_id = qsq.query_id 
			INNER JOIN ' + @DatabaseName + '.sys.query_store_query_text AS qsqt ON qsqt.query_text_id = qsq.query_text_id 
			WHERE query_plan_hash = ' + @QueryPlanHashStr;

	EXEC sp_executesql @stmt = @sql, @params = N'@QueryPlan NVARCHAR(MAX) OUTPUT, @QuerySQLText NVARCHAR(MAX) OUTPUT', @QueryPlan = @QueryPlan OUTPUT, @QuerySQLText = @QuerySQLText OUTPUT;

	UPDATE #OutputMissing
		SET QueryPlan = @QueryPlan, SampleQueryText = @QuerySQLText
...
SELECT DatabaseName,
       ...
       'CREATE NONCLUSTERED INDEX [IX_' + SUBSTRING(TableName, 2, LEN(TableName) - 2) + '_' 
		+ REPLACE(REPLACE(REPLACE(ISNULL(EqualityColumns,'') + CASE WHEN InequalityColumns IS NOT NULL THEN '_' ELSE '' END + ISNULL(InequalityColumns,''), '[', ''), ']',''), ', ','_')
		+ CASE WHEN IncludeColumns IS NOT NULL THEN '_includes' ELSE '' END
		+ '] ON '
		+ '[' + DatabaseName + '].' + SchemaName + '.' + TableName 
		+ ' ( ' + IsNull(EqualityColumns, '') 
		+ CASE WHEN InequalityColumns IS NULL THEN '' ELSE 
			CASE WHEN EqualityColumns IS NULL THEN '' ELSE ',' END 
		+ InequalityColumns END + ' ) ' 
		+ CASE WHEN IncludeColumns IS NULL THEN '' ELSE 'INCLUDE (' + IncludeColumns + ')' END 
	...
	FROM #OutputMissing
	WHERE DatabaseName NOT IN ('WISE', 'SectraHealthcareStorage', 'SectraDeploymentStorage', 'SOURCE_PACS', 'SOURCE_BIS')
