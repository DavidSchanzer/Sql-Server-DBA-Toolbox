...
BEGIN
    ...
    IF @WithMoveDataFiles <> ''
        SET @WithMoveDataFiles_ = @WithMoveDataFiles;
    DECLARE @WithMoveLogFile_ VARCHAR(2000);
    IF @WithMoveLogFile <> ''
        SET @WithMoveLogFile_ = @WithMoveLogFile;
    DECLARE @WithMoveFileStreamFile_ VARCHAR(2000);
    IF @WithMoveFileStreamFile <> ''
        SET @WithMoveFileStreamFile_ = @WithMoveFileStreamFile;
    ...
    IF @TargetDatabase_ IS NOT NULL
       AND @Database_ IS NULL
    ...
    IF ISNULL(@WithMoveDataFiles, '') <> ''
       OR ISNULL(@WithMoveLogFile_, '') <> ''
       OR ISNULL(@WithMoveFileStreamFile_, '') <> ''
       OR ISNULL(@TargetDatabase_, '') <> ''
    ...
    IF @IncludeDeviceType7 = 1
        INSERT INTO #DeviceTypes
        (
            device_type
        )
        SELECT 7;
    IF @IncludeDeviceType102 = 1
        INSERT INTO #DeviceTypes
        (
            device_type
        )
        SELECT 102;
    IF @IncludeDeviceType2 = 1
        INSERT INTO #DeviceTypes
        (
            device_type
        )
        SELECT 2;
    IF @IncludeDeviceType9 = 1
        INSERT INTO #DeviceTypes
        (
            device_type
        )
    ...
    BEGIN
        ...
        AS (SELECT 1 AS StartPosition,
            ...
            SELECT CAST(EndPosition AS INT) + 1 AS StartPosition,
                   ...
                   SUBSTRING(
                                @Database_,
                                EndPosition + 1,
                                ISNULL(NULLIF(CHARINDEX(',', @Database_, EndPosition + 1), 0), LEN(@Database_) + 1)
                                - EndPosition - 1
                            ) AS DatabaseItem
            FROM Databases1
        ...
        AS (SELECT CASE
                       WHEN DatabaseItem LIKE '-%' THEN
                           RIGHT(DatabaseItem, LEN(DatabaseItem) - 1)
                       ELSE
                           DatabaseItem
                   END AS DatabaseItem,
                   CASE
        ...
        AS (SELECT CASE
                   ...
                   END AS DatabaseItem,
                   CASE
                       WHEN DatabaseItem = 'SYSTEM_DATABASES' THEN
                           'S'
                       WHEN DatabaseItem = 'USER_DATABASES' THEN
                           'U'
                       ELSE
                           NULL
                   END AS DatabaseType,
        ...
        AS (SELECT CASE
                       WHEN LEFT(DatabaseItem, 1) = '['
                            AND RIGHT(DatabaseItem, 1) = ']' THEN
                           PARSENAME(DatabaseItem, 1)
                       ELSE
                           DatabaseItem
                   END AS DatabaseItem,
                   DatabaseType,
        ...
        IF @Database_ IS NULL OR NOT EXISTS (SELECT * FROM @SelectedDatabases)
           OR EXISTS
        (
            SELECT *
        ...
        BEGIN
            SET @ErrorMessage
                = N'The value for the parameter @Databases is not supported.' + CHAR(13) + CHAR(10) + N' ';
    ...
    WHERE a.[type] = 'D'
          ...
          (
              a.database_name IN
              (
                  SELECT DatabaseName FROM #tmpDatabases WHERE Selected = 1
              )
              OR @Database_ IS NULL
              OR a.database_name = @Database_
          )
          AND device_type IN
              (
                  SELECT device_type FROM #DeviceTypes
              )
    ...
    SELECT bs.database_name,
           ...
           MIN(fork_point_lsn) AS fork_point_lsn
    INTO #BackupFork
    ...
    -- CTE1 Full Backup 
    --------------------------------------------------------------
    WITH CTE (database_name, current_compatibility_level, first_lsn, last_lsn, fork_point_lsn, current_is_read_only,
              current_state_desc, current_recovery_model_desc, has_backup_checksums, backup_size, [type],
              backupmediasetid, family_sequence_number, backupfinishdate, physical_device_name, position,
    ...
    AS (
       ...
       FROM msdb.dbo.backupset bs
           ...
           JOIN msdb.dbo.backupmediafamily mf
               ON mf.media_set_id = bs.media_set_id
                  AND mf.family_sequence_number
                  BETWEEN bs.first_family_number AND bs.last_family_number
       WHERE bs.type = 'D'
             AND mf.mirror = 0
       ...
       FROM msdb.dbo.backupset bs
           ...
           (
               SELECT DISTINCT
                      x.database_name,
                      CASE
                          WHEN y.last_Log_After_StopAt IS NULL THEN
                              CONVERT(DATETIME, '31 Dec 2050')
                          ELSE
                              y.last_Log_After_StopAt
                      END AS last_Log_After_StopAt
       ...
       WHERE bs.backup_start_date <= x.last_Log_After_StopAt -- Include 1st log after stop at
             ...
             AND
             (
                 bf.fork_point_lsn > bs.first_lsn
                 OR bf.first_recovery_fork_guid IS NULL
                 OR bs.first_recovery_fork_guid <> bs.last_recovery_fork_guid
             ))
    SELECT *
    INTO #CTE
    FROM CTE;
    ...
    --------------------------------------------------------------
    WITH Stripes (database_name, backupmediasetid, family_sequence_number, last_lsn, S2_pdn, S3_pdn, S4_pdn, S5_pdn,
                  S6_pdn, S7_pdn, S8_pdn, S9_pdn, S10_pdn, S11_pdn, S12_pdn, S13_pdn, S14_pdn, S15_pdn
                 )
    AS (SELECT Stripe1.database_name,
    ...
    SELECT CASE
               WHEN @RestoreScriptReplaceThis_ IS NOT NULL
                    AND @RestoreScriptWithThis_ IS NOT NULL THEN
                   REPLACE(a.Command, @RestoreScriptReplaceThis_, @RestoreScriptWithThis_)
               ELSE
                   a.Command
           END AS TSQL, --Allow substitution of a path
           CONVERT(NVARCHAR(30), a.backupfinishdate, 126) AS BackupDate,
           a.BackupDevice,
           a.first_lsn,
           a.last_lsn,
           a.fork_point_lsn,
    ...
    (
        ...
        SELECT 'RESTORE DATABASE [' + CASE ISNULL(@TargetDatabase_, '')
                                          WHEN '' THEN
                                              d.[name]
                                          ELSE
                                              @TargetDatabase_
                                      END + ']' + SPACE(1) + CHAR(13)
               + CASE
                     WHEN #CTE.physical_device_name LIKE 'http%' THEN
                         ' FROM URL = '
                     ELSE
                         ' FROM DISK = N'
                 END + ''''
               + --Support restore from blob storage in Azure
            ...
            CASE ISNULL(@FromFileFullUNC_, 'Actual')
                ...
                ELSE
                    ...
                    + SUBSTRING(
                                   #CTE.physical_device_name,
                                   LEN(#CTE.physical_device_name)
                                   - CHARINDEX('\', REVERSE(#CTE.physical_device_name), 1) + 2,
                                   CHARINDEX('\', REVERSE(#CTE.physical_device_name), 1) + 1
                               )
            END + '''' + SPACE(1) + CHAR(13)
               +
                ...
                ELSE
                    ...
                    + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                          ...
                          ELSE
                              ...
                              + SUBSTRING(
                                             #Stripes.S2_pdn,
                                             LEN(#Stripes.S2_pdn) - CHARINDEX('\', REVERSE(#Stripes.S2_pdn), 1) + 2,
                                             CHARINDEX('\', REVERSE(#Stripes.S2_pdn), 1) + 1
                                         )
                      END + ''''
            END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S3_pdn,
                                                  LEN(#Stripes.S3_pdn) - CHARINDEX('\', REVERSE(#Stripes.S3_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S3_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S4_pdn,
                                                  LEN(#Stripes.S4_pdn) - CHARINDEX('\', REVERSE(#Stripes.S4_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S4_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S5_pdn,
                                                  LEN(#Stripes.S5_pdn) - CHARINDEX('\', REVERSE(#Stripes.S5_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S5_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S6_pdn,
                                                  LEN(#Stripes.S6_pdn) - CHARINDEX('\', REVERSE(#Stripes.S6_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S6_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S7_pdn,
                                                  LEN(#Stripes.S7_pdn) - CHARINDEX('\', REVERSE(#Stripes.S7_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S7_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S8_pdn,
                                                  LEN(#Stripes.S8_pdn) - CHARINDEX('\', REVERSE(#Stripes.S8_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S8_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S9_pdn,
                                                  LEN(#Stripes.S9_pdn) - CHARINDEX('\', REVERSE(#Stripes.S9_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S9_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S10_pdn,
                                                  LEN(#Stripes.S10_pdn) - CHARINDEX('\', REVERSE(#Stripes.S10_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S10_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S11_pdn,
                                                  LEN(#Stripes.S11_pdn) - CHARINDEX('\', REVERSE(#Stripes.S11_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S11_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S12_pdn,
                                                  LEN(#Stripes.S12_pdn) - CHARINDEX('\', REVERSE(#Stripes.S12_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S12_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S13_pdn,
                                                  LEN(#Stripes.S13_pdn) - CHARINDEX('\', REVERSE(#Stripes.S13_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S13_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S14_pdn,
                                                  LEN(#Stripes.S14_pdn) - CHARINDEX('\', REVERSE(#Stripes.S14_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S14_pdn), 1) + 1
                                              )
                           END + ''''
                 END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileFullUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S15_pdn,
                                                  LEN(#Stripes.S15_pdn) - CHARINDEX('\', REVERSE(#Stripes.S15_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S15_pdn), 1) + 1
                                              )
                           END + ''''
                 END + ' WITH ' + CASE
                                      WHEN #CTE.physical_device_name LIKE 'http%'
                                           AND @BlobCredential_ IS NOT NULL THEN
                                          ' CREDENTIAL = ''' + @BlobCredential_ + ''', '
                                      ELSE
                                          ''
                                  END + CASE ISNULL(@WithReplace_, 0)
                                            WHEN 1 THEN
                                                'REPLACE, '
                                            ELSE
                                                ''
                                        END + 'FILE = ' + CAST(#CTE.position AS VARCHAR(5)) + ','
               + CASE #CTE.has_backup_checksums
                     WHEN 1 THEN
                         'CHECKSUM, '
                     ELSE
                         ' '
                     ...
                     ELSE
                         ...
                         + ISNULL(
                                     ...
                                     SUBSTRING(
                                                  ...
                                                  1,
                                                  LEN(#CTE.physical_device_name)
                                                  - CHARINDEX('\', REVERSE(#CTE.physical_device_name))
                                              )
                                 ) + '\' + d.name + '_'
                         + REPLACE(REPLACE(SUBSTRING(CONVERT(VARCHAR(24), GETDATE(), 127), 12, 12), ':', ''), '.', '')
                         + '_ROLLBACK_UNDO.bak ' + '''' + ','
                 END + SPACE(1) + 'STATS=10' + SPACE(1)
               + CASE @SuppressWithMove_
                     WHEN 0 THEN
                         ...
                         + CASE ISNULL(@WithMoveDataFiles_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  x.PhysicalName,
                                                  LEN(x.PhysicalName) - CHARINDEX('\', REVERSE(x.PhysicalName), 1) + 2,
                                                  CHARINDEX('\', REVERSE(x.PhysicalName), 1) + 1
                                              )
                           END + '''' + SPACE(1) + ', MOVE N' + '''' + y.LogicalName + '''' + ' TO ' + ''''
                         + CASE ISNULL(@WithMoveLogFile_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  y.PhysicalName,
                                                  LEN(y.PhysicalName) - CHARINDEX('\', REVERSE(y.PhysicalName), 1) + 2,
                                                  CHARINDEX('\', REVERSE(y.PhysicalName), 1) + 1
                                              )
                           END + ''''
                     ELSE
                         ' '
        ...
        FROM sys.databases d
            ...
            (
                SELECT DB_NAME(mf.database_id) AS name,
                       CASE ISNULL(@TargetDatabase_, '')
                           WHEN '' THEN
                               mf.physical_name
                           ELSE
                               SUBSTRING(   mf.physical_name,
                                            1,
                                            CASE CHARINDEX('.', mf.physical_name)
                                                WHEN 0 THEN
                                                    LEN(mf.physical_name)
                                                ELSE
                                                    CHARINDEX('.', mf.physical_name) - 1
                                            END
                                        ) + '_' + @TargetDatabase_ + '.mdf'
                       END AS PhysicalName,
            ...
            END
               ...
               + CASE ISNULL(#Stripes.S15_pdn, '')
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileDiffUNC_, 'Actual')
                           ...
                           END + ''''
                 END + ' WITH FILE = ' + CAST(#CTE.position AS VARCHAR(5)) + ',' + CASE #CTE.has_backup_checksums
                                                                                       WHEN 1 THEN
                                                                                           'CHECKSUM, '
                                                                                       ELSE
                                                                                           ' '
               ...
               + CASE @StandbyMode_
                     ...
                     ELSE
                         ...
                         + REPLACE(REPLACE(SUBSTRING(CONVERT(VARCHAR(24), GETDATE(), 127), 12, 12), ':', ''), '.', '')
                         + '_ROLLBACK_UNDO.bak ' + '''' + ','
                 END + SPACE(1) + 'STATS=10' + SPACE(1) AS Command,
               750000 AS Sequence,
               d.name AS database_name,
               #CTE.physical_device_name AS BackupDevice,
               #CTE.backupfinishdate,
        ...
        SELECT 'RESTORE LOG [' + CASE ISNULL(@TargetDatabase_, '')
                                     WHEN '' THEN
                                         d.[name]
                                     ELSE
                                         @TargetDatabase_
                                 END + ']' + SPACE(1) + CHAR(13) + CASE
                                                                       WHEN #CTE.physical_device_name LIKE 'http%' THEN
                                                                           ' FROM URL = '
                                                                       ELSE
                                                                           ' FROM DISK = N'
            ...
            END
                     ...
                     ELSE
                         ...
                         + CASE ISNULL(@FromFileLogUNC_, 'Actual')
                               ...
                               ELSE
                                   ...
                                   + SUBSTRING(
                                                  #Stripes.S11_pdn,
                                                  LEN(#Stripes.S10_pdn) - CHARINDEX('\', REVERSE(#Stripes.S11_pdn), 1)
                                                  + 2,
                                                  CHARINDEX('\', REVERSE(#Stripes.S11_pdn), 1) + 1
                                              )
                           END + ''''
               ...
               + CASE @StandbyMode_
                 ...
                 END + SPACE(1) + CASE
                                      ...
                                      ELSE
                                          ''
                                  END + CASE #CTE.has_backup_checksums
                                            WHEN 1 THEN
                                                ' CHECKSUM, '
                                            ELSE
                                                ' '
                                        END + +'FILE = ' + CAST(#CTE.position AS VARCHAR(5)) + ' ,STOPAT = ' + ''''
               + CONVERT(VARCHAR(21), @StopAt_, 120) + '''' AS Command,
               1000000 AS Sequence,
               d.name AS database_name,
               #CTE.physical_device_name AS BackupDevice,
               #CTE.backupfinishdate,
        ...
        WHERE #CTE.[type] = 'L'
              AND #CTE.family_sequence_number = 1
              AND #CTE.last_lsn >= CASE
                                       WHEN @ExcludeDiffAndLogBackups_ = 0 THEN
                                           ISNULL(after_diff.last_lsn, '0')
                                       ELSE
                                           0
                                   END
        ...
        --------------------------------------------------------------
        SELECT 'BEGIN TRY ALTER DATABASE [' + CASE ISNULL(@TargetDatabase_, '')
                                                  WHEN '' THEN
                                                      d.[name]
                                                  ELSE
                                                      @TargetDatabase_
                                              END + ']'
               + ' SET SINGLE_USER WITH ROLLBACK IMMEDIATE END TRY BEGIN CATCH PRINT' + '''' + 'SET SINGLE USER FAILED'
        ...
        SELECT 'RESTORE DATABASE [' + CASE ISNULL(@TargetDatabase_, '')
                                          ...
                                          ELSE
                                              @TargetDatabase_
                                      END + ']' + SPACE(1) + 'WITH RECOVERY' AS Command,
               1000001 AS Sequence,
               d.name AS database_name,
               '' AS BackupDevice,
               GETDATE() AS backupfinishdate,
        ...
        UNION -- CHECKDB
        --------------------------------------------------------------
        SELECT 'DBCC CHECKDB(' + '''' + CASE ISNULL(@TargetDatabase_, '')
                                            WHEN '' THEN
                                                d.[name]
                                            ELSE
                                                @TargetDatabase_
                                        END + '''' + ') WITH NO_INFOMSGS, ALL_ERRORMSGS' AS Command,
               1000002 AS Sequence,
               d.name AS database_name,
               '' AS BackupDevice,
        ...
        SELECT
            ...
            'DROP DATABASE [' + CASE ISNULL(@TargetDatabase_, '')
                                    WHEN '' THEN
                                        d.[name]
                                    ELSE
                                        @TargetDatabase_
                                END + ']' + SPACE(1) AS Command,
            1000003 AS Sequence,
            d.name AS database_name,
            '' AS BackupDevice,
            GETDATE() AS backupfinishdate,
        ...
        --------------------------------------------------------------
        SELECT 'RAISERROR (' + ''''
               + 'Multiple restores per performed between the selected full backup and stop at time for database '
               + CASE ISNULL(@TargetDatabase_, '')
                     WHEN '' THEN
                         d.[name]
                     ELSE
                         @TargetDatabase_
                 END + ' - Restore Gene processing aborted.' + '''' + ',0,0);' + +SPACE(1) AS Command,
               1 AS Sequence,
               d.name AS database_name,
               '' AS BackupDevice,
               GETDATE() AS backupfinishdate,
                          ...
                          WHEN 'Actual' THEN
                              CASE ISNULL(@TargetDatabase_, '')
                                  WHEN '' THEN
                                      b.physical_name
                                  ELSE
                                      SUBSTRING(   b.physical_name,
                                                   1,
                                                   CASE CHARINDEX('.', b.physical_name)
                                                       WHEN 0 THEN
                                                           LEN(b.physical_name)
                                                       ELSE
                                                           CHARINDEX('.', b.physical_name) - 1
                                                   END
                                               ) + '_' + @TargetDatabase_ + '.ndf'
                              END
                          ELSE
                              CASE ISNULL(@TargetDatabase_, '')
                                  WHEN '' THEN
                                      ...
                                      + SUBSTRING(
                                                     b.physical_name,
                                                     LEN(b.physical_name) - CHARINDEX('\', REVERSE(b.physical_name), 1)
                                                     + 2,
                                                     CHARINDEX('\', REVERSE(b.physical_name), 1) + 1
                                                 )
                                  ELSE
                                      @WithMoveDataFiles_
                                      + SUBSTRING(
                                                     b.physical_name,
                                                     (LEN(b.physical_name) - CHARINDEX('\', REVERSE(b.physical_name))
                                                      + 2
                                                     ),
                                                     CHARINDEX('\', REVERSE(b.physical_name)) - 1
    ...
    WHERE (
              @Database_ IS NULL
              OR b.Selected = 1
              OR @Database_ = a.database_name
          )
          AND
          (
              @IncludeSystemDBs_ = 1
              OR a.database_name NOT IN ( 'master', 'model', 'msdb' )
          )
          AND a.last_lsn >= @LogShippingLastLSN_;

    CREATE INDEX IDX_RestoreGeneResults
    ON #RestoreGeneResults (
                               database_name,
                               SortSequence,
                               BackupDate
                           );
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT x4.TSQL,
                   ...
                   x4.last_lsn,
                   x4.fork_point_lsn,
                   x4.first_recovery_fork_guid,
                   x4.last_recovery_fork_guid,
                   x4.database_name,
                   x4.SortSequence
            FROM #RestoreGeneResults x4
            WHERE ISNULL(x4.BackupDevice, '') <> 'SECONDARY FULL'
            ORDER BY x4.database_name,
                     x4.SortSequence,
                     x4.BackupDate;
        END;
        ELSE
            ...
            BEGIN
                ...
                AS (SELECT last_lsn,
                           STUFF(
                                    (
                                        SELECT ' ' + TSQL
                                        FROM #RestoreGeneResults x3
                                        WHERE x3.last_lsn = x2.last_lsn
                                              AND ISNULL(x3.BackupDevice, '') = 'SECONDARY FULL'
                                        ORDER BY x3.SortSequence
                                        FOR XML PATH, TYPE
                    ...
                    GROUP BY last_lsn)
                SELECT CASE @SuppressWithMove_
                           WHEN 0 THEN
                               CASE ISNULL(x5.WithoveCmds, '')
                                   WHEN '' THEN
                                       x4.TSQL
                                   ELSE
                                       x4.TSQL + ' ' + x5.WithoveCmds
                               END
                           ELSE
                               x4.TSQL
                       END AS TSQL,
